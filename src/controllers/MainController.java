...
public class MainController implements Serializable{
	...
	//search fields for check
	@FXML protected TextField bookSearcC;
	@FXML protected TextField memberSearchC;

	// search button for check
	@FXML protected Button searchBC;
	@FXML protected Button searchMC;
	...
	//ListView on CheckOut
		...
		void serachMemberof(){
			...
			for(LibraryMember l: listOfLibMember) {
				//to do process here
				if(l.getMembId().equals(memberSearchC.getText())){
					memList.add(l);
				}
			}
		...
		void searchBookof(){
			...
			for(Book b: lb) {
				if(b.isAvailable() && b.getNumCop() > 0){
					if(b.getTitle().contains(bookSearcC.getText())){
						bookList.add(b);
					}
